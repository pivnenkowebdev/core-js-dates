...
function getNextFriday(date) {
  ...
  const currentDayOfWeek = currDay.getDay();
...
function getCountDaysInMonth(month, year) {
  const testDate = new Date(year, month);
  const currMonth = testDate.setDate(0);
  return new Date(currMonth).getDate();
}
...
function formatDate(date) {
  const currDay = new Date(date);
  const newFormat = currDay.toLocaleString('en', { timeZone: 'UTC' });
  return newFormat;
}
...
function getQuarter(date) {
  ...
  let quarter = 0;
  if (indexMonth >= 0 && indexMonth <= 2) {
    quarter = 1;
  } else if (indexMonth >= 3 && indexMonth <= 5) {
    quarter = 2;
  } else if (indexMonth >= 6 && indexMonth <= 8) {
    quarter = 3;
  } else if (indexMonth >= 9 && indexMonth <= 11) {
    quarter = 4;
  }
  return quarter;
}
